...
#include "EUTeltripletFinder.h"
#include "EUTelNav.h"

namespace eutelescope {

EUTeltripletFinder::EUTeltripletFinder():  
...
std::vector<EUTelTrack>& EUTeltripletFinder::getTracks()
{
	return	_finalTracks; 
}

void EUTeltripletFinder::testTrackCandidates(){
...
void EUTeltripletFinder::propagateForwardFromSeedState(EUTelState& stateInput, EUTelTrack& track)
{
	...
	std::cout<<"Hello Helen, this is EUTripletFinder"<<std::endl;
...
void EUTeltripletFinder::setRadLengths(EUTelTrack & track,	std::map<const int,double>  mapSensor, std::map<const int ,double>  mapAir, double rad ){
...
void EUTeltripletFinder::printTrackCandidates(){
...
void EUTeltripletFinder::clearFinalTracks(){
	_finalTracks.clear();
}
...
void EUTeltripletFinder::testTrackQuality()
{
...
void EUTeltripletFinder::testPositionEstimation(float position1[], float position2[]){
...
void EUTeltripletFinder::findTrackCandidatesWithSameHitsAndRemove(){
	streamlog_out(MESSAGE1) << "EUTeltripletFinder::findTrackCandidatesWithSameHitsAndRemove----BEGIN" << std::endl;
...
//		cout<<"Enter the last addition of track"<<std::endl;
	...
	streamlog_out(MESSAGE1) << "------------------------------EUTeltripletFinder::findTrackCandidatesWithSameHitsAndRemove()---------------------------------END" << std::endl;
}
...
void EUTeltripletFinder::testUserInput() {
	streamlog_out(DEBUG2) << "EUTeltripletFinder::testUserInput()" << std::endl;
	if ( _beamE < 1.E-6 ) {
...
void EUTeltripletFinder::initialiseSeeds()
{
...
TVector3 EUTeltripletFinder::computeInitialMomentumGlobal(){
...
void EUTeltripletFinder::testInitialSeeds(){
...
void EUTeltripletFinder::findTrackCandidates() {
	streamlog_out(MESSAGE1) << "EUTeltripletFinder::findTrackCandidates()" << std::endl;
	...
	streamlog_out(MESSAGE1) << "EUTeltripletFinder::findTrackCandidates()------END" << std::endl;
}

//We need to delete the states AND the track. Since we have allocated memory to store the state.
void EUTeltripletFinder::clearTrackAndTrackStates(){
	for(size_t i=0; i < _tracks.size(); ++i){
		for(size_t j=0; j< _tracks.at(i).getTracks().size(); ++j){   
...
void EUTeltripletFinder::findTracksWithEnoughHits(){
	streamlog_out(DEBUG1) << "EUTeltripletFinder::findTracksWithEnoughHits()------BEGIN" << std::endl;
	...
	streamlog_out(DEBUG1) << "EUTeltripletFinder::findTracksWithEnoughHits()-----END" << std::endl;

}
//This creates map between plane ID and hits on that plane. 
//We also order the map correcly with geometry.
void EUTeltripletFinder::setHitsVecPerPlane()
{
...
void EUTeltripletFinder::setPlaneDimensionsVec(EVENT::IntVec planeDimensions){
...
void EUTeltripletFinder::testHitsVecPerPlane(){
...
void EUTeltripletFinder::testPlaneDimensions(){
     ...
     * 
	 */
const EVENT::TrackerHit* EUTeltripletFinder::findClosestHit(EUTelState& state)
{
...
double EUTeltripletFinder::getXYPredictionPrecision(EUTelState& /*ts*/) const 
{
...
* @return 
*/
TVectorD EUTeltripletFinder::computeResidual(EUTelState& state, const EVENT::TrackerHit* hit) const
{
...
void EUTeltripletFinder::printHits()
{
	streamlog_out(MESSAGE0) << "EUTeltripletFinder::prinitHit: BEGIN ==============" << std::endl;
	EVENT::TrackerHitVec::const_iterator itHit;
	...
	streamlog_out(MESSAGE0) << "EUTeltripletFinder::printHits: END ==============" << std::endl;
}
