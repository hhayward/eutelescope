...
void EUTelProcessorPatRecTriplets::plotHistos( std::vector<EUTelTrack>& trackCandidates)  {
  ...
  for (size_t i = 0; i< trackCandidates.size( ) ; ++i ) {//loop over all tracks
    static_cast < AIDA::IHistogram1D* > ( _aidaHistoMap1D[ _histName::_chi2CandidateHistName ] ) -> fill(trackCandidates[i].getChi2());
	...
	continue;
      ...
      if(sensorID==1){
	streamlog_out( MESSAGE2 ) << "trying to locate central hit. x = "<<trackCandidates[i].getStates()[j].getHit().getPosition()[0]<<std::endl;
	//trackCandidates[i].getStates()[j]// is doublet information stored on the track object ? How do i retrieve it here?
      }
      else if(sensorID==4){}// will this work when there are DUTs?
    }
...
void EUTelProcessorPatRecTriplets::bookHistograms() {
  ...
  try {
    ...
    const int tracksNBin = 40;    
    const double tracksXMin = -0.5;
    const double tracksXMax = 39.5;
